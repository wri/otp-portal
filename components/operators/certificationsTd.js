...
const CertificationsTd = ({ fmus }) => {
  ...
  const pbn = fmus.filter(f => f['certification-pbn']).length;
  ...
  if (!fsc && !olb && !pefc && !pafc && !fsccw && !tlv && !ls && !pbn) {
    return (
...
CertificationsTd.defaultProps = {
  fmus: []
};

CertificationsTd.propTypes = {
